-- CreateEnum
CREATE TYPE "Role" AS ENUM ('patient', 'doctor', 'secretary', 'admin');

-- CreateEnum
CREATE TYPE "TokenType" AS ENUM ('PASSWORD_RESET', 'EMAIL_OTP');

-- CreateTable
CREATE TABLE "User" (
    "id" TEXT NOT NULL,
    "email" TEXT NOT NULL,
    "password" TEXT NOT NULL,
    "role" "Role" NOT NULL DEFAULT 'patient',
    "is_2fa_enabled" BOOLEAN NOT NULL DEFAULT false,
    "two_fa_secret" TEXT,
    "name" TEXT NOT NULL,
    "image" TEXT NOT NULL DEFAULT 'data:image/png;base64,',
    "address" JSONB DEFAULT '{"line1":"","line2":""}',
    "gender" TEXT NOT NULL DEFAULT 'Not Selected',
    "dob" TEXT NOT NULL DEFAULT 'Not Selected',
    "phone" TEXT NOT NULL DEFAULT '0000000000',
    "speciality" TEXT,
    "degree" TEXT,
    "experience" TEXT,
    "about" TEXT,
    "available" BOOLEAN NOT NULL DEFAULT true,
    "fees" DOUBLE PRECISION,
    "slots_booked" JSONB NOT NULL DEFAULT '{}',
    "data" DOUBLE PRECISION,

    CONSTRAINT "User_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "AuthToken" (
    "id" TEXT NOT NULL,
    "user_id" TEXT NOT NULL,
    "token_or_code" TEXT NOT NULL,
    "type" "TokenType" NOT NULL,
    "expires_at" TIMESTAMP(3) NOT NULL,
    "is_used" BOOLEAN NOT NULL DEFAULT false,

    CONSTRAINT "AuthToken_pkey" PRIMARY KEY ("id")
);

-- CreateTable
CREATE TABLE "Appointment" (
    "id" TEXT NOT NULL,
    "userId" TEXT NOT NULL,
    "docId" TEXT NOT NULL,
    "slotDate" TEXT NOT NULL,
    "slotTime" TEXT NOT NULL,
    "userData" JSONB NOT NULL,
    "docData" JSONB NOT NULL,
    "amount" DOUBLE PRECISION NOT NULL,
    "data" DOUBLE PRECISION NOT NULL,
    "cancelled" BOOLEAN NOT NULL DEFAULT false,
    "payment" BOOLEAN NOT NULL DEFAULT false,
    "isCompleted" BOOLEAN NOT NULL DEFAULT false,

    CONSTRAINT "Appointment_pkey" PRIMARY KEY ("id")
);

-- CreateIndex
CREATE UNIQUE INDEX "User_email_key" ON "User"("email");

-- AddForeignKey
ALTER TABLE "AuthToken" ADD CONSTRAINT "AuthToken_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Appointment" ADD CONSTRAINT "Appointment_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Appointment" ADD CONSTRAINT "Appointment_docId_fkey" FOREIGN KEY ("docId") REFERENCES "User"("id") ON DELETE CASCADE ON UPDATE CASCADE;
